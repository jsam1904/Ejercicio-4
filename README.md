...
La API quedará disponible en `http://localhost:24546`.
...
curl http://localhost:24546/api/items

# Obtener equipo con ID 1
curl http://localhost:24546/api/items/1

# Crear un equipo
curl -X POST http://localhost:24546/api/items \
...
curl -X PATCH http://localhost:24546/api/items/1 \
  -H "Content-Type: application/json" \
  -d '{"titulos":33}'

# Eliminar un equipo
curl -X DELETE http://localhost:24546/api/items/1
```
